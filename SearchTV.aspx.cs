...
public partial class SearchTV : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        else
        {
            ...
            con.Open();
            string query = "select series_name, image, writer_name, director_name, producer_name, release_date, description from tvs where series_name='" + Session["tv"] + "'";
            cmd = new SqlCommand(query, con);
            dr = cmd.ExecuteReader();
            if (dr.Read())
            {
                lblMsg.Text = dr.GetString(0).ToString();
                txtTVName.Text = dr.GetString(0).ToString();
